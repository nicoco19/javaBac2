...
public class ExerciceGroupingBy {
    ...
    private void groupBy2() {
        ...
        Map<Trader, Long> traderListMap = transactions
                .stream()
    ...
    private void groupBy3() {
        ...
        Map<TransactionsLevel,List<Transaction>> traderListMap = transactions
                .stream()
                .collect(groupingBy(transaction -> {

                    if (transaction.getValue() >= 1000) return TransactionsLevel.VERY_HI;
                    if (transaction.getValue() >= 800 && transaction.getValue() < 1000) return TransactionsLevel.HI;
                    if (transaction.getValue() >= 600 && transaction.getValue() < 800) return TransactionsLevel.ME;
