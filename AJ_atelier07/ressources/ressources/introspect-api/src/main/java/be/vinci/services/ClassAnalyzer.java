...
public class ClassAnalyzer {
     ...
     * @return
     */
    public JsonObject getFullInfo() throws NoSuchFieldException {
        JsonObjectBuilder objectBuilder = Json.createObjectBuilder();
        objectBuilder.add("name", aClass.getSimpleName());
        objectBuilder.add("fields", getFields());
        objectBuilder.add("methods",getMethodes());
     ...
     */
    public JsonArray getFields() throws NoSuchFieldException {
    ...
    public JsonObject getMethode(Method m) {
        JsonObjectBuilder objectBuilderM = Json.createObjectBuilder();
        objectBuilderM.add("name",m.getName());
        objectBuilderM.add("returnType",m.getReturnType().getName());
        objectBuilderM.add("parametres",getMethodeParam(m));
        objectBuilderM.add("visibility", getMethoddVisibility(m));
        objectBuilderM.add("isStatic",isMethodeStatic(m) );
        objectBuilderM.add("isAbstract",isMethodeMAbstract(m));
        return objectBuilderM.build();
    }

    private boolean isMethodeStatic(Method m) {
        return Modifier.isStatic(m.getModifiers());
    }
    private boolean isMethodeMAbstract(Method m) {
        return Modifier.isAbstract(m.getModifiers());
    }

    private String getMethoddVisibility(Method m) {
    ...
    public JsonArray getMethodes() throws NoSuchFieldException {
        JsonArrayBuilder arrayBuilder = Json.createArrayBuilder();

        for (Method m : aClass.getDeclaredMethods()) {

            arrayBuilder.add(getMethode(m));

        }
        return arrayBuilder.build();
    }

    private JsonArray getMethodeParam(Method method){
        JsonArrayBuilder arrayBuilder = Json.createArrayBuilder();
        for (Parameter param : method.getParameters()) {

            arrayBuilder.add(param.getType().getSimpleName());
        }
        return arrayBuilder.build();
    }
